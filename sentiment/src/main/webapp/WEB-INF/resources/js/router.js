// Filename: router.js
define([ 'jquery', 'underscore', 'backbone', 'backgrid', 'backgrid-paginator', 'backgrid-select-all', 'bootstrap.min',
		'globals/global'
	...
	var initialize = function() {
		...
		app_router.on('route:getSentimentList', function() {
			console.log("getting the list of Sentiments.");
			require([ 'collections/SentimentCollection', 'views/SentimentCollectionView' ], function(
					SentimentCollection, SentimentCollectionView) {
				var collectionToFetch = new SentimentCollection();
				collectionToFetch.fetch({
					success : function(collectionData) {
						Global.showView(new SentimentCollectionView({
							el : $("#bodyContainer"),
