...
(function(root, $, _, Backbone) {
	...
	if (!String.prototype.trim || ws.trim()) {
		...
		ws = "[" + ws + "]";
		var trimBeginRegexp = new RegExp("^" + ws + ws + "*"), trimEndRegexp = new RegExp(
				ws + ws + "*$");
	...
	_.extend(NumberFormatter.prototype, {
		...
		fromRaw : function(number) {
			...
			var decimalPart = parts[1] ? (this.decimalSeparator || '.')
					+ parts[1] : '';

			return integerPart.replace(this.HUMANIZED_NUM_RE, '$1'
					+ this.orderSeparator)
					+ decimalPart;
		},
	...
	_.extend(DatetimeFormatter.prototype, {
		...
		_convert : function(data, validate) {
			...
			if (_.isNumber(data)) {
				...
				time = lpad(jsDate.getUTCHours(), 2, 0) + ':'
				...
				time = date && parts[1] ? parts[1] : this.TIME_RE
			...
			var jsDate = new Date(Date.UTC(YYYYMMDD[1] * 1 || 0,
					YYYYMMDD[2] * 1 - 1 || 0, YYYYMMDD[3] * 1 || 0,
					HHmmssSSS[1] * 1 || null, HHmmssSSS[2] * 1 || null,
					HHmmssSSS[3] * 1 || null, HHmmssSSS[5] * 1 || null));

			var result = '';

			if (this.includeDate) {
				result = lpad(jsDate.getUTCFullYear(), 4, 0) + '-'
						+ lpad(jsDate.getUTCMonth() + 1, 2, 0) + '-'
						+ lpad(jsDate.getUTCDate(), 2, 0);
			}

			if (this.includeTime) {
				result = result + (this.includeDate ? 'T' : '')
				...
				if (this.includeMilli) {
					result = result + '.'
							+ lpad(jsDate.getUTCMilliseconds(), 3, 0);
				}
			}
	...
	_.extend(SelectFormatter.prototype, {
		 ...
		 */
		fromRaw : function(rawValue) {
			return _.isArray(rawValue) ? rawValue
	 ...
	 */
	var CellEditor = Backgrid.CellEditor = Backbone.View
			.extend({
				...
				initialize : function(options) {
					Backgrid.requireOptions(options, [ "formatter", "column",
							"model" ]);
					...
					this.listenTo(this.model, "backgrid:editing",
							this.postRender);
				},
				...
				postRender : function(model, column) {
					if (column == null
							|| column.get("name") == this.column.get("name")) {
						this.$el.focus();
					}
					return this;
				}
	 ...
	 */
	var InputCellEditor = Backgrid.InputCellEditor = CellEditor
			.extend({
				 ...
				 */
				render : function() {
					this.$el.val(this.formatter.fromRaw(this.model
							.get(this.column.get("name"))));
					return this;
				},
				...
				saveOrCancel : function(e) {
					...
					if (command.moveUp() || command.moveDown()
						...
						if (_.isUndefined(newValue)) {
							...
							model.trigger("backgrid:edited", model, column,
									command);
						}
					}
					// esc
						...
						e.stopPropagation();
						model
								.trigger("backgrid:edited", model, column,
										command);
					}
				},
	...
	/**
	 ...
	 @extends Backbone.View
	 */
	var Cell = Backgrid.Cell = Backbone.View
			.extend({
				...
				initialize : function(options) {
					...
					this.formatter = Backgrid.resolveNameToClass(this.column
							.get("formatter")
							|| this.formatter, "Formatter");
					this.editor = Backgrid.resolveNameToClass(this.editor,
							"CellEditor");
					this.listenTo(this.model, "change:"
							+ this.column.get("name"), function() {
				...
				render : function() {
					this.$el.empty();
					this.$el.text(this.formatter.fromRaw(this.model
							.get(this.column.get("name"))));
					this.delegateEvents();
					return this;
				},
				...
				enterEditMode : function() {
					...
					if (column.get("editable")) {
						...
						model.trigger("backgrid:edit", model, column, this,
								this.currentEditor);
						...
						model.trigger("backgrid:editing", model, column, this,
								this.currentEditor);
					}
				},
	...
	var DatetimeCell = Backgrid.DatetimeCell = Cell.extend({
		...
		initialize : function(options) {
			...
			this.editor = this.editor.extend({
				attributes : _.extend({}, this.editor.prototype.attributes,
	...
	var BooleanCellEditor = Backgrid.BooleanCellEditor = CellEditor.extend({
		 ...
		 */
		render : function() {
			var val = this.formatter.fromRaw(this.model.get(this.column
					.get("name")));
		...
		saveOrCancel : function(e) {
			...
			var $el = this.$el;
			if (command.save() || command.moveLeft() || command.moveRight()
	...
	var BooleanCell = Backgrid.BooleanCell = Cell.extend({
		...
		render : function() {
			...
			this.$el.append($("<input>", {
				tabIndex : -1,
				type : "checkbox",
				checked : this.formatter.fromRaw(this.model.get(this.column
						.get("name")))
			}));
			this.delegateEvents();
			return this;
	...
	/**
	 ...
	 @extends Backgrid.CellEditor
	 */
	var SelectCellEditor = Backgrid.SelectCellEditor = CellEditor
			.extend({
				...
				template : _
						.template('<option value="<%- value %>" <%= selected ? \'selected="selected"\' : "" %>><%- text %></option>'),
				...
				render : function() {
					...
					var selectedValues = this.formatter.fromRaw(this.model
							.get(this.column.get("name")));
					...
					for (var i = 0; i < optionValues.length; i++) {
						...
						if (_.isArray(optionValue)) {
							...
							optgroup.append(this._renderOptions(
									optionValue.values, selectedValues));
				...
				close : function(e) {
					...
					if (command.cancel()) {
						...
						model.trigger("backgrid:edited", model, column,
							...
							|| command.moveDown() || e.type == "blur") {
						e.preventDefault();
						e.stopPropagation();
						if (e.type == "blur"
								&& this.$el.find("option").length === 1) {
							model.set(column.get("name"), this.formatter
									.toRaw(this.$el.val()));
						}
						model.trigger("backgrid:edited", model, column,
								new Command(e));
	...
	/**
	 ...
	 @extends Backgrid.Cell
	 */
	var SelectCell = Backgrid.SelectCell = Cell
			.extend({
				...
				initialize : function(options) {
					...
					this.listenTo(this.model, "backgrid:edit", function(model,
							column, cell, editor) {
				...
				render : function() {
					...
					var rawData = this.formatter.fromRaw(this.model
							.get(this.column.get("name")));
	...
	var Column = Backgrid.Column = Backbone.Model.extend({
		...
		initialize : function(attrs) {
			...
			var headerCell = Backgrid.resolveNameToClass(
					this.get("headerCell"), "HeaderCell");
	...
	var Row = Backgrid.Row = Backbone.View.extend({
		...
		initialize : function(options) {
			...
			this.listenTo(columns, "change:renderable", function(column,
					renderable) {
	...
	var HeaderCell = Backgrid.HeaderCell = Backbone.View.extend({
		...
		initialize : function(options) {
			...
			this.listenTo(this.collection, "backgrid:sort",
					this._resetCellDirection);
		},
		...
		/**
		   ...
		   @private
		 */
		_resetCellDirection : function(sortByColName, direction, comparator,
				collection) {
		...
		sort : function(columnName, direction, comparator) {
			...
			this.collection.trigger("backgrid:sort", columnName, direction,
					comparator, this.collection);
		},
		...
		render : function() {
			this.$el.empty();
			var $label = $("<a>").text(this.column.get("label")).append(
	...
	/**
	 ...
	 @extends Backbone.View
	 */
	var Body = Backgrid.Body = Backbone.View
			.extend({
				 ...
				 */
				initialize : function(options) {
					Backgrid.requireOptions(options,
							[ "columns", "collection" ]);
					...
					this.listenTo(collection, "backgrid:edited",
							this.moveToNextCell);
				},
				...
				insertRow : function(model, collection, options) {
					...
					if (this.rows[0] instanceof EmptyRow)
				...
				moveToNextCell : function(model, column, command) {
					...
					if (command.moveUp() || command.moveDown()
						...
						if (command.moveUp() || command.moveDown()) {
							...
							if (row)
								row.cells[j].enterEditMode();
						} else if (command.moveLeft() || command.moveRight()) {
							var right = command.moveRight();
							for (var offset = i * l + j + (right ? 1 : -1); offset >= 0
									&& offset < maxOffset; right ? offset++
									: offset--) {
								var m = ~~(offset / l);
								var n = offset - m * l;
								var cell = this.rows[m].cells[n];
								if (cell.column.get("renderable")
										&& cell.column.get("editable")) {
									cell.enterEditMode();
									break;
								}
							}
	...
	var Grid = Backgrid.Grid = Backbone.View.extend({
		...
		initialize : function(options) {
			...
			this.listenTo(this.columns, "reset", function() {
				this.header = new (this.header.remove().constructor)(
						passedThruOptions);
				this.body = new (this.body.remove().constructor)(
						passedThruOptions);
				if (this.footer) {
