...
(function(factory) {
	...
	// Browser
		...
		var PageableCollection = Backbone.PageableCollection = factory(_,
				Backbone);
		...
		(function(_, Backbone) {
			 ...
			 */
			var PageableCollection = Backbone.Collection
					.extend({
						...
						initialize : function(models, options) {
							...
							var state = this.state = _extend({},
									PageableProto.state, this.state,
									options.state || {});

							state.currentPage = state.currentPage == null ? state.firstPage
									: state.currentPage;

							if (mode != "server" && state.totalRecords == null
									&& !_isEmpty(models)) {
							...
							if (state.sortKey && !comparator) {
								this.setSorting(state.sortKey, state.order,
										options);
							}
						...
						_makeCollectionEventHandler : function(pageCol, fullCol) {
							...
							return function collectionEventHandler(event,
								...
								var currentPage = firstPage === 0 ? state.currentPage
										: state.currentPage - 1;
								var pageSize = state.pageSize;
								var pageStart = currentPage * pageSize, pageEnd = pageStart
										+ pageSize;
								...
								if (event == "add") {
									...
									if (collection == fullCol) {
										...
										if (fullIndex >= pageStart
												&& fullIndex < pageEnd) {
											colToAdd = pageCol;
											pageIndex = addAt = fullIndex
													- pageStart;
										}
									} else {
										...
										colToAdd = fullCol;
										var addAt = !_isUndefined(options.at) ? options.at
												+ pageStart
												: fullIndex;
									}
									...
									if (colToAdd) {
										...
										var modelToRemove = pageIndex >= pageSize ? model
												: !_isUndefined(options.at)
														&& addAt < pageEnd
														&& pageCol.length > pageSize ? pageCol
														.at(pageSize)
														: null;
										if (modelToRemove) {
									...
									if (!options.onAdd) {
										...
										if (!--state.totalRecords) {
											...
											var totalPages = state.totalPages = ceil(state.totalRecords
													/ pageSize);
											state.lastPage = firstPage === 0 ? totalPages - 1
													: totalPages;
											if (state.currentPage > totalPages)
												state.currentPage = state.lastPage;
										}
										pageCol.state = pageCol
												._checkState(state);
								...
								if (event == "reset" || event == "sort") {
									...
									if (collection == pageCol
											&& event == "reset") {
										var head = fullCol.models.slice(0,
												pageStart);
										var tail = fullCol.models
												.slice(pageStart
														+ pageCol.models.length);
										options = _extend(options, {
											silent : true
										});
										resetQuickly(fullCol, head.concat(
												pageCol.models).concat(tail),
												options);
									}
									...
									if (event == "reset"
										...
										pageCol.state = pageCol
												._checkState(state);
										if (collection == pageCol)
											fullCol.trigger(event, fullCol,
													options);
										resetQuickly(pageCol, fullCol.models
						...
						_checkState : function(state) {
							...
							if (totalRecords != null && pageSize != null
									&& currentPage != null && firstPage != null
									&& (mode == "infinite" ? links : true)) {

								totalRecords = finiteInt(totalRecords,
										"totalRecords");
								pageSize = finiteInt(pageSize, "pageSize");
								currentPage = finiteInt(currentPage,
										"currentPage");
								...
								totalPages = state.totalPages = ceil(totalRecords
										/ pageSize);
								...
								state.lastPage = firstPage === 0 ? max(0,
										totalPages - 1) : totalPages;

								if (mode == "infinite") {
									if (!links[currentPage + '']) {
										throw new RangeError(
												"No link found for page "
														+ currentPage);
									}
								} else {
									if (firstPage === 0
											&& (currentPage < firstPage || (currentPage >= totalPages && totalPages > 0))) {
										throw new RangeError(
						...
						setPageSize : function(pageSize, options) {
							...
							this.state = this
									._checkState(_extend(
											...
											{
												pageSize : pageSize,
												totalPages : ceil(this.state.totalRecords
														/ pageSize)
											}));

							return this
									.getPage(this.state.currentPage, options);
						},
						...
						switchMode : function(mode, options) {
							...
							if (!_contains([ "server", "client", "infinite" ],
									mode)) {
								throw new TypeError(
							...
							var state = this.state = options.resetState ? _clone(this._initState)
							...
							if (mode != "server" && !fullCollection) {
								fullCollection = this
										._makeFullCollection(options.models
												|| []);
								fullCollection.pageableCollection = this;
								this.fullCollection = fullCollection;
								var allHandler = this
										._makeCollectionEventHandler(this,
												fullCollection);
								_each([ "add", "remove", "reset", "sort" ],
										function(event) {
											handlers[event] = handler = _.bind(
													allHandler, {}, event);
							...
							if (mode == "infinite") {
								var links = this.links = {};
								var firstPage = state.firstPage;
								var totalPages = ceil(state.totalRecords
										/ state.pageSize);
								var lastPage = firstPage === 0 ? max(0,
										totalPages - 1) : totalPages
										|| firstPage;
							...
							return options.fetch ? this.fetch(_omit(options,
									"fetch", "resetState")) : this;
						},
						...
						getPage : function(index, options) {
							...
							var pageStart = (firstPage === 0 ? pageNum
									: pageNum - 1)
									* pageSize;
							var pageModels = fullCollection
									&& fullCollection.length ? fullCollection.models
									.slice(pageStart, pageStart + pageSize)
									: [];
							if ((mode == "client" || (mode == "infinite" && !_isEmpty(pageModels)))
									&& !options.fetch) {
								return resetQuickly(this, pageModels, _omit(
										options, "fetch"));
							}
						...
						sync : function(method, model, options) {
							...
							if (self.mode == "infinite") {
								...
								options.success = function(resp, status, xhr) {
									...
									var newLinks = self.parseLinks(resp,
											_extend({
												xhr : xhr
											}, options));
									if (newLinks.first)
						...
						parseLinks : function(resp, options) {
							var linkHeader = options.xhr
									.getResponseHeader("Link");
							var relations = [ "first", "prev", "previous",
									"next", "last" ];
							var links = {};
							_each(linkHeader.split(","),
									function(linkValue) {
										var linkParts = linkValue.split(";");
										var url = linkParts[0].replace(
												URL_TRIM_RE, '');
										var params = linkParts.slice(1);
										_each(params, function(param) {
											var paramParts = param.split("=");
											var key = paramParts[0].replace(
													PARAM_TRIM_RE, '');
											var value = paramParts[1].replace(
													PARAM_TRIM_RE, '');
											if (key == "rel"
													&& _contains(relations,
															value)) {
							...
							if (qs = (qsi = last.indexOf('?')) ? last
								...
								if (!totalRecords) {
									if (pageNum)
										totalRecords = (state.firstPage === 0 ? pageNum + 1
												: pageNum)
												* pageSize;
									else if (totalPages)
						 ...
						 */
						parse : function(resp) {
							var newState = this.parseState(resp,
									_clone(this.queryParams),
									_clone(this.state));
							if (newState)
								this.state = this._checkState(_extend({},
										this.state, newState));
						...
						parseState : function(resp, queryParams, state) {
							if (resp && resp.length === 2
									&& _.isObject(resp[0]) && _isArray(resp[1])) {

								var newState = _clone(state);
								var serverState = resp[0];

								_each(_pairs(_omit(queryParams, "directions")),
										function(kvp) {
											var k = kvp[0], v = kvp[1];
											var serverVal = serverState[v];
											if (!_isUndefined(serverVal)
													&& !_.isNull(serverVal))
						...
						fetch : function(options) {
							...
							// dedup query params
							var url = _result(options, "url")
									|| _result(this, "url") || '';
							var qsi = url.indexOf('?');
							if (qsi != -1) {
								_extend(data, queryStringToParams(url
										.slice(qsi + 1)));
								url = url.slice(0, qsi);
							}
							...
							var queryParams = this.mode == "client" ? _pick(
									this.queryParams, "sortKey", "order")
									: _omit(_pick(this.queryParams,
							...
							if (mode != "server") {
								...
								options.success = function(col, resp, opts) {
									...
									if (mode == "client")
										resetQuickly(fullCollection, models,
												opts);
									else if (links[currentPage]) { // refetching a page
										var pageSize = state.pageSize;
										var pageStart = (state.firstPage === 0 ? currentPage
												: currentPage - 1)
												* pageSize;
										var fullModels = fullCollection.models;
										var head = fullModels.slice(0,
												pageStart);
										var tail = fullModels.slice(pageStart
												+ pageSize);
										fullModels = head.concat(models)
												.concat(tail);
										var updateFunc = fullCollection.set
												|| fullCollection.update;
										updateFunc.call(fullCollection,
												fullModels, _extend({
													silent : true,
													sort : false
												}, opts));
								...
								return BBColProto.fetch.call(self, _extend({},
										options, {
											silent : true
										}));
							}
						...
						_makeComparator : function(sortKey, order) {
							...
							return function(left, right) {
								var l = left.get(sortKey), r = right
										.get(sortKey), t;
								if (order === 1)
									t = l, l = r, r = t;
								if (l === r)
						...
						setSorting : function(sortKey, order, options) {
							...
							var comparator = this._makeComparator(sortKey,
									order);
