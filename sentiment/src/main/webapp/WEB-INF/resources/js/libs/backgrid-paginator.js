...
(function($, _, Backbone, Backgrid) {
	 ...
	 */
	Backgrid.Extension.Paginator = Backbone.View
			.extend({
				...
				/** @property */
				template : _
						.template('<ul><% _.each(handles, function (handle) { %><li <% if (handle.className) { %>class="<%= handle.className %>"<% } %>><a href="#" <% if (handle.title) {%> title="<%= handle.title %>"<% } %>><%= handle.label %></a></li><% }); %></ul>'),
				...
				changePage : function(e) {
					...
					if (!$li.hasClass("active") && !$li.hasClass("disabled")) {
						...
						var pageIndex = +label;
						collection
								.getPage(state.firstPage === 0 ? pageIndex - 1
										: pageIndex);
					}
				},
				...
				makeHandles : function() {
					...
					var currentPage = Math.max(state.currentPage,
							state.firstPage);
					currentPage = firstPage ? currentPage - 1 : currentPage;
					var windowStart = Math.floor(currentPage / this.windowSize)
							* this.windowSize;
					var windowEnd = Math.min(lastPage + 1, windowStart
							+ this.windowSize);
					...
					if (ffLabels) {
						...
						if (ffLabels.prev) {
							handles.unshift({
								label : ffLabels.prev,
								className : collection.hasPrevious() ? void 0
										: "disabled"
							});
						}
						...
						if (ffLabels.next) {
							handles.push({
								label : ffLabels.next,
								className : collection.hasNext() ? void 0
										: "disabled"
							});
						}
